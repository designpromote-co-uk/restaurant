...
add_action( "wp_enqueue_scripts", "zn_enqueue", 11 );
function zn_enqueue() {
...
add_action( 'wp_head', 'zn_flexslider' );
//add_action( 'wp_enqueue_scripts', 'zn_flexslider' ); //no working
function zn_flexslider() {
	if( is_home() || is_front_page() )
	_e( "<script>
	//<![CDATA[
...
add_action( 'wp_loaded', 'zn_home_widgets' ); //use wp_loaded to load widgets in order
function zn_home_widgets() {
...
add_action( 'wp_loaded', 'zn_footer_widgets' );
function zn_footer_widgets() {
...
add_action( 'wp_loaded', 'zn_head_widgets' );
function zn_head_widgets() {
...
//add_filter( 'the_content', 'zn_nl2br' );
// convert breakline to <br>
function zn_nl2br( $content ) {
	if( is_page( 'menu' ) )
...
add_action( 'admin_menu', 'zn_slider_menu_page_theme' );

function zn_slider_menu_page_theme(){
	//$hook = add_menu_page( 'ZN Sliders title', 'Sliders', 'manage_options', 'zn_slider', 'zn_slider_list' );
	$hook = add_theme_page( 'ZN Sliders title', 'Sliders', 'manage_options', 'zn_slider', 'zn_slider_list' );
	
	add_submenu_page( 'zn_slider', 'Upload New Slider', 'Add New', 'manage_options', 'zn_new_slider', 'zn_slider_upload' );
	add_action( "load-$hook", 'zn_add_options' );
}

function zn_add_options() {
	global $sliderListTable;
	$sliderListTable = new zn_slider_List_Table();
}

class zn_slider_List_Table extends WP_List_Table {
	private $slider_data;
...
function zn_slider_help($contextual_help, $screen_id, $screen) {
...
add_filter('contextual_help', 'zn_slider_help', 10, 3);

function znslider_admin_bar_menu(){
	global $wp_admin_bar;
...
add_action('wp_before_admin_bar_render', 'znslider_admin_bar_menu');

/**
* upload file and call zn_insert_attachment
*/
function zn_media_handle_upload($file_id, $post_id, $post_data = array(), $overrides = array( 'test_form' => false )) {
	...
	// Save the data
	$id = zn_insert_attachment($attachment, $file, $post_id);
	if ( !is_wp_error($id) ) {
...
*/
function zn_insert_attachment($object, $file = false, $parent = 0) {
	global $wpdb, $user_ID;
...
<?php
		...
		if( $_POST ) {
			...
			if ( isset($_POST['slider-upload']) && !empty($_FILES) ) {
				...
				$id = zn_media_handle_upload( 'slider-file', $post_id );
				//var_dump( $id );
				if ( is_wp_error( $id ) )
					$location .= '?message=3';
